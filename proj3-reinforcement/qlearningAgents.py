...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        QValue  = features.__mul__(self.weights)
    ...
    def update(self, state, action, nextState, reward):
        ...
        if not nextStateActions:
            nextQValue = 0.0
        else:
            nextQValue = max(self.getQValue(nextState, action) for action in nextStateActions)
        
        diff = (reward + self.discount * nextQValue) - self.getQValue(state, action)
        features = self.featExtractor.getFeatures(state, action)
